			...
			<table class="w-full whitespace-no-wrap">
				...
				>
					...
					<td class="border-t">
						<div
							class="px-6 py-4 flex items-center focus:text-indigo-500"
							:style="{ 'color': contract.payable == 'Atrasado' ? 'red' : contract.paid ? 'green' : null }"
						>
							{{ contract.payable }}
						</div>
					</td>
					<td class="border-t w-px">
						<inertia-link
							v-show="!contract.paid"
							class="px-4 flex items-center"
							:href="route('rental_payment.create', { month: form.month, year: form.year, contract })"
							tabindex="-1"
						>
							Lançar pagamento
						</inertia-link>
    ...
    data() {
        return {
            form: {
                ...
                month: this.filters.month || (new Date().getMonth() + 1),
